...
def test_fallback_state_machine(monkeypatch, mock_registry, tmp_path):
    ...
    monkeypatch.setattr("src.implementer.create_worktree", lambda *a: tmp_path)
    monkeypatch.setattr("src.implementer.get_hermes_execution_handoff", lambda **kwargs: (True, "model", "ok", {}))
    monkeypatch.setattr("src.implementer.get_reports_dir", lambda *a: tmp_path)
...
def test_alias_models_sorted_after_concrete(mock_registry):
    import time
    from src.implementation_models import load_registry, save_registry, get_eligible_models
