...
@patch('src.autonomous_guard.validate_hermes_execution')
@patch('src.hermes_agent.research')
@patch('src.hermes_agent.plan')
@patch('builtins.open')
def test_generate_daily_digest_hermes_trigger(mock_open, mock_plan, mock_research, mock_preflight, mock_config, mock_prereq, mock_score, mock_conn):
    # Setup mock db
...
@patch("src.implementer.get_agent_config")
@patch("builtins.open")
def test_generate_daily_digest_skips_hermes_when_preflight_fails(
    mock_open,
    mock_config,
    mock_prereq,
    mock_score,
    ...
    mock_cursor = MagicMock()
    mock_cursor.fetchall.side_effect = [
        [("url1", "repoA", 125, "Issue 3", "body", "orgA")],
        [], [], [], [], []
    ]
    mock_cursor.fetchone.side_effect = [(10,), (0,), (0,)]
    mock_conn.return_value.__enter__.return_value.cursor.return_value = mock_cursor
    ...
    mock_prereq.return_value = ("READY_NOW", "ev")
    mock_config.return_value = {"hermes_auto_trigger_threshold": 80.0}
    mock_preflight.return_value = (False, "provider unavailable")
